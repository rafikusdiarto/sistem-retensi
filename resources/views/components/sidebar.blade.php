<aside
    ...
    <div class="h-19">
        <i class="absolute top-0 right-0 p-4 opacity-50 cursor-pointer fas fa-times text-slate-400 xl:hidden"
            sidenav-close></i>
        <a class="block px-8 py-6 text-sm whitespace-nowrap text-slate-700 flex">
            <img src="{{asset('./assets/img/profile.webp')}}"
                class="inline rounded-full h-full max-w-full transition-all duration-200 ease-nav-brand max-h-8" alt="main_logo" />

            <div class="ml-2">
                <p class="ml-1 font-bold transition-all text-center uppercase duration-200 ease-nav-brand">{{ Auth::user()->name }}</p>
                <p class="ml-1 font-semibold transition-all text-slate-400 text-center duration-200 ease-nav-brand -mt-5">{{ Auth::user()->getRoleNames()->first() }}</p>
            </div>
        </a>
    </div>
            ...
            <ul class="flex flex-col pl-0 mb-0">
                <li class="mt-0.5 w-full">
                    <a href="{{route('dashboardPetugas')}}"
                        class="py-2.7 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap rounded-lg px-4 transition-colors {{ (request()->segment(1) == 'dashboard-petugas') ? 'bg-menu text-white' : '' }}">
                        <div
                            class="mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-2.5">
                            <i class="relative top-0 text-sm leading-normal text-blue-500 ni ni-tv-2"></i>
                        </div>
                ...
                <li class="mt-0.5 w-full">
                    <a href="{{ route('laporanRetensi') }}"
                        class="py-2.7 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap rounded-lg px-4 transition-colors {{ (request()->segment(1) == 'laporan-retensi') ? 'bg-menu text-white' : '' }}">
                        <div
                            class="mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-2.5">
                            <i class="relative top-0 text-sm leading-normal text-pink-600 ni ni-ruler-pencil"></i>
                        </div>
                ...
                <li class="mt-0.5 w-full">
                    <a href="{{route('sopRetensi')}}"
                        class="py-2.7 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap rounded-lg px-4 transition-colors {{ (request()->segment(1) == 'sop-retensi') ? 'bg-menu text-white' : '' }}">
                        <div
                            class="mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-2.5">
                            <i class="relative top-0 text-sm leading-normal text-yellow-400 ni ni-badge"></i>
                        </div>
                ...
                <li class="mt-0.5 w-full">
                    <a href="{{route('arsipRekamMedis')}}"
                        class="  py-2.7 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap rounded-lg px-4 transition-colors {{ (request()->segment(1) == 'arsip-rm') ? 'bg-menu text-white' : '' }}">
                        <div
                            class="mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-2.5">
